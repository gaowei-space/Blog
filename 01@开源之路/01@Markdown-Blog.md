...
- 执行 `make run` 启动 web 服务，运行之后访问地址 http://localhost:5006，API请求会转发给 markdown-blog
- 编译 `make`

- 打包 `make package`，在 package 目录生成当前系统的压缩包 markdown-blog-v0.0.5-darwin-amd64.tar
- 运行 `make package-all` 生成 Windows、Linux、Mac 的压缩包 至 **package** 文件夹
